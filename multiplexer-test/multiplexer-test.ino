...
uint8_t vlmap(int range, int rangeMin, int rangeMax);
...
Serial.print("wire begin");
  ...
  Wire.begin();
Serial.print("wire begin aftern");
  Serial.println("Looking for sensor vl1");

  /* Initialise the 1st sensor */
  if (!vlbegin(&vl1,vl1Port)) {
    return;
  }
  Serial.println("vl1 working ");
  Serial.println("Looking for sensor vl2");

  /* Initialise the 2nd sensor */
  if (!vlbegin(&vl2,vl2Port)) {
    return;
  }
    Serial.print("vl2 working ");
